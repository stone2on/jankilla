...
{
    class ShortTagRepository : BaseTagRepository
    {
        public ShortTagRepository(string connString) : base(connString)
        {
        }
        ...
        {
            var tags = new List<ShortTag>();

            var sql = $"SELECT * " +
                $"FROM Tags WHERE Discriminator = {(int)ETagDiscriminator.Short}";
            using (var connection = new SqlConnection(ConnectionString))
            {
                connection.Open();
                tags.AddRange(connection.Query<ShortTag>(sql));
            }
        ...
        public override IEnumerable<Tag> GetAll(Block parent)
        {
            ...
            var sql = $"SELECT * FROM Tags WHERE Discriminator = {(int)ETagDiscriminator.Short} AND BlockID = '{parent.ID}'";
            using (var connection = new SqlConnection(ConnectionString))
            {
                connection.Open();
                tags.AddRange(connection.Query<ShortTag>(sql));
