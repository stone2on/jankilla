...
namespace Jankilla.Driver.Mitsubishi.MxComponent
{
    public class MitsubishiMxComponentBlock : Block
    {
        ...
        public override string Discriminator => nameof(MitsubishiMxComponentBlock);
        ...
        public override ValidationResult ValidateTag(Tag tag)
        {
            if (tag.Address == null)
            {
                return new ValidationResult(false, "Tag address is null.");
            }
            ...
            if (!bParsed)
            {
                return new ValidationResult(false, "Failed to parse the address number.");
            }

            if (!tag.Address.StartsWith(this.DeviceCode))
            {
                return new ValidationResult(false, "Address does not start with the expected device code.");
            }
